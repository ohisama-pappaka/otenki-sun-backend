...
def fetch_city_list(prefecture_name: str):
    ...
    # ファイル内に存在するデータをもとにした DB の操作
    if prefecture_name in db.prefecture_db.prefecture_db:
        return list(db.prefecture_db.prefecture_db[prefecture_name].keys())
    else:
...
def fetch_city_data(prefecture_name: str, city_name: str):
    ...
    # ファイル内に存在するデータをもとにした DB の操作
    if prefecture_name in db.prefecture_db.prefecture_db:
        if city_name in db.prefecture_db.prefecture_db[prefecture_name]:
            sub_id = int(
                db.prefecture_db.prefecture_db[prefecture_name][city_name]["id"]
            )
            sub_name = db.prefecture_db.prefecture_db[prefecture_name][city_name][
                "city_name"
            ]

            # sub_id の値の調整
            sub_id = "{0:06d}".format(sub_id)
